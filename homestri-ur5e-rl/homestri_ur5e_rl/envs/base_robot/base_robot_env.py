...
class BaseRobot(MujocoEnv):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        observation_space = spaces.Box(low=-np.inf, high=np.inf, shape=(34,), dtype=np.float64)
        # 加了一个eef位置和目标位置，各3维 28+6
        ...
        self.action_real_low = np.array([-2, -2, -2, -1, -1, -1, 0])
        self.action_real_high = np.array([2, 2, 2, 1, 1, 1, 255])
        ...
        # )
    def normalize_action(self, action_real: np.array) -> np.ndarray:
        action_real = np.clip(action_real, self.action_real_low, self.action_real_high)
        action_norm = 2 * (action_real - self.action_real_low) / (self.action_real_high - self.action_real_low) - 1
        return action_norm
    def denormalize_action(self, action_norm: np.ndarray) -> np.ndarray:
        ...
        action_real = (action_norm + 1) / 2 * (self.action_real_high - self.action_real_low) + self.action_real_low
        return action_real
    def step(self, action):
        ...
        if self.is_training:
            ...
            print("normalization:", action)
        else:
            action = action.copy()
            print("without normalization:", action)
        ...
        obs = self._get_obs()
        distance = np.linalg.norm(self.eef_pos - self.goal_pos)
        reward = -distance
        
        # Check both goal and max steps conditions
        done = distance < 0.5
        if self._steps >= 10:
            truncated = True
        else:
    ...
    def _get_obs(self):
        ...
        print("joint_names:", self.model_names.joint_names)
        print("robot_qpos:", robot_qpos)
        print("robot_qvel:", robot_qvel)
        # Get the end-effector position and orientation
        self.eef_pos = self.data.site_xpos[self.model_names.site_name2id["robot0:eef_site"]]
        # eef_quat = self.data.site_xquat[self.model_names.site_name2id["robot0:eef_site"]]
        # print("eef_pos:", self.eef_pos)
        # print("goal_pos:", self.goal_pos)
        return np.concatenate((robot_qpos.copy(), robot_qvel.copy(), self.eef_pos, self.goal_pos))
    def set_goal_pos(self, pos: np.ndarray):
        """Set the position of the goal sphere using mocap."""
        # For mocap bodies, we directly set the position in data.mocap_pos
        self.data.mocap_pos[0] = pos  # Assuming goal is the first mocap body
        self.data.mocap_quat[0] = np.array([1, 0, 0, 0])  # Identity quaternion
    ...
    def reset_model(self):
        ...
        self.goal_pos = np.array([self.np_random.uniform(-1.2, 1.2),
                                 self.np_random.uniform(-1.2, 1.2), 
                                 self.np_random.uniform(0.8, 1.2)])
        # Update goal sphere's position
        self.set_goal_pos(self.goal_pos)
        obs = self._get_obs()
